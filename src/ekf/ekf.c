...
void EKF_Init(EKF_Context *ctx, uint16_t n, uint16_t m, float32_t *work_mem, 
    ...
    arm_mat_init_f32(&ctx->T1,   n, n, p); p += n * n;
    arm_mat_init_f32(&ctx->T2,   n, n, p); p += n * n;
    arm_mat_init_f32(&ctx->K,    n, m, p); p += n * m;
    arm_mat_init_f32(&ctx->S,    m, m, p); p += m * m;
    arm_mat_init_f32(&ctx->invS, m, m, p); p += m * m;
    arm_mat_init_f32(&ctx->HT,   n, m, p); p += n * m;
    arm_mat_init_f32(&ctx->HP,   m, n, p);

    ctx->f = NULL; ctx->h = NULL;
}
